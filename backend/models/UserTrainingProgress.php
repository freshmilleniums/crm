...
class UserTrainingProgress extends \yii\db\ActiveRecord
{
    ...
    {
        $totalActive = self::getActiveModulesCount();
        if ($totalActive === 0) {
            return false;
        }
        return count($this->getCompletedModulesArray()) >= $totalActive;
    }

    /**
     * Total count of active training modules
     */
    public static function getActiveModulesCount(): int
    {
        return TrainingModule::find()->where(['is_active' => 1])->count();
    }
}