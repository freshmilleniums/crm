...
class UserSearch extends User
{
    ...
    public function rules()
    {
        return [
            ...
            [['is_online'], 'boolean'],
            [['created_from', 'created_to', 'q'], 'safe'],
        ];
    }
    ...
    public function search($params, $formName = null)
    {
        ...
        if ($this->created_from) {
            $query->andWhere(['>=', 'user.created_at', strtotime($this->created_from)]);
        }
        if ($this->created_to) {
            $query->andWhere(['<=', 'user.created_at', strtotime($this->created_to . ' 23:59:59')]);
        }
        ...
        if (!empty($this->q)) {
            $query->andWhere([
                'or',
                ['like', 'user.first_name',   $this->q],
                ['like', 'user.last_name',    $this->q],
                ['like', 'user.email',        $this->q],
                ['like', 'user.phone_number', $this->q],
                ['like', "CONCAT(user.first_name, ' ', user.last_name)", $this->q],
            ]);
        }
