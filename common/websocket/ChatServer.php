...
class ChatServer
{
    ...
    public function handleClose($connection)
    {
        ...
        if ($connectionData && $connectionData['userId']) {
            ...
            if (isset($this->userConnections[$userId][$connectionId])) {
                ...
                echo "Removed connection {$connectionId} for user {$userId}\n";
            }
            ...
            if (empty($this->userConnections[$userId])) {
                ...
                echo "User {$userId} fully disconnected\n";
            } else {
                echo "User {$userId} still has " . count($this->userConnections[$userId]) . " connections\n";
            }
        }
    ...
    protected function handleAuth($connection, $message)
    {
        ...
        // Reconnect DB to avoid "MySQL server has gone away"
        try {
            Yii::$app->db->close();
            Yii::$app->db->open();
        } catch (\Exception $e) {
            echo "DB reconnect failed: " . $e->getMessage() . "\n";
        }
        ...
        $connectionId = spl_object_id($connection);
        $this->userConnections[$userId][$connectionId] = $connection;
        echo "User {$userId} now has " . count($this->userConnections[$userId]) . " connections\n";
        ...
        try {
            ...
            // Clean up connection from userConnections before closing
            $connectionId = spl_object_id($connection);
            if (isset($this->userConnections[$userId][$connectionId])) {
                unset($this->userConnections[$userId][$connectionId]);
            }
    ...
    protected function handleNewMessage($message)
    {
        ...
        foreach ($participants as $userId) {
            ...
            if (isset($this->userConnections[$userId])) {
                ...
                echo "Message sent to all connections of user {$userId}\n";
            } else {
        ...
        try {
            if (in_array($userRole, ['super-administrator', 'administrator'])) {
                ...
                $privateChats = Chat::find()
                    ...
                    ->where([
                        'chats.type' => Chat::TYPE_USER_PRIVATE,
                        'cp.user_id' => $userId,
                        'cp.left_at' => null
                    ])
                    ->select('chats.id')
                    ->column();
                $chatIds = array_merge($employeeChats, $privateChats);
            } elseif ($userRole === 'employee') {
                // Employee sees only their own support chat
                $chatIds = Chat::find()
                    ->where(['type' => Chat::TYPE_EMPLOYEE, 'employee_id' => $userId])
                    ->select('id')
                    ->column();
            } else {
                // phone-operator, email-task-operator — only private chats where participant
                $chatIds = Chat::find()
                    ->innerJoin('chat_participants cp', 'chats.id = cp.chat_id')
                    ->where([
                        'chats.type' => Chat::TYPE_USER_PRIVATE,
