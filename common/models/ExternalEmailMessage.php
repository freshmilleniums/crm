 ...
 * @property ExternalEmailAttachment[] $attachments
 * @property ExternalEmailReadStatus[] $readStatuses
 */
class ExternalEmailMessage extends ActiveRecord
{
    ...
    {
        return '{{%external_email_messages}}';
    }
    ...
    {
        return [
            [['email_account_id', 'direction', 'from_email', 'to_emails'], 'required'],

            [['email_account_id', 'is_draft', 'has_attachments', 'sent_at', 'received_at'], 'integer'],
            ...
            [['is_draft', 'has_attachments'], 'boolean'],
            [['is_draft', 'has_attachments'], 'default', 'value' => 0],
    ...
    protected function generateMessageId()
    {
        ...
        $domain = $emailAccount ? explode('@', $emailAccount->email)[1] : 'localhost';
    ...
    {
        return $this->hasMany(ExternalEmailAttachment::class, ['message_id' => 'id']);
    }

    public function getReadStatuses()
    {
        return $this->hasMany(ExternalEmailReadStatus::class, ['message_id' => 'id']);
    }

    public function isReadByUser($userId)
    {
        $status = ExternalEmailReadStatus::find()
