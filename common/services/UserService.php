...
class UserService
{
    ...
    private const CALL_CENTER_STATUS_GROUPS = [
        'not_processed' => [
            'label' => 'Not Processed',
            'substatuses' => [1]  // SUBSTATUS_NOT_PROCESSED
        ],
        'in_progress' => [
            'label' => 'In Progress',
            'substatuses' => [2, 3, 4, 5, 6, 7, 11]  // NEW_APPLICANT, PRIMARY_CONTACT, INTERVIEW_COMPLETED, CONTRACT_SENT, TRAINING, FINAL_ASSIGNMENT, UNCOMPLETED_TASK
        ],
        'active_employees' => [
            'label' => 'Active Employees',
            'substatuses' => [8]  // SUBSTATUS_ACTIVE_EMPLOYEE
        ...
        'archived' => [
            'label' => 'Archived / Refused',
            'substatuses' => [9, 10]  // CONTRACT_REFUSED, ARCHIVED
        ],
    ];
    ...
    public function getAvailableRoleKeysForUser(int $userId): array
    {
        ...
        return match ($userRoleKey) {
            ...
            'administrator' => [
                'employee','phone-operator', 'email-task-operator'
            ],
            default => []
        };
    }
     ...
     * @return array
     */
    public function getTabsDataForRoles(int $status = User::STATUS_ACTIVE): array
    {
        ...
        foreach ($availableRoles as $roleName => $roleDisplayName) {
            ...
            $dataProvider = $this->createDataProviderForRole($roleName, $status, $filterModel);
     ...
     */
    private function createDataProviderForRole(string $roleName, int $status = User::STATUS_ACTIVE, UserSearch $filterModel = null): ActiveDataProvider
    {
        if ($filterModel !== null) {
            ...
            if (in_array($roleName, ['super-administrator', 'administrator'])) {
                $dataProvider->query->andWhere(['user.company_id' => 0]);
            }

            $dataProvider->sort->attributes = array_merge($dataProvider->sort->attributes, [
                'id'           => ['asc' => ['user.id' => SORT_ASC],           'desc' => ['user.id' => SORT_DESC]],
                'first_name'   => ['asc' => ['user.first_name' => SORT_ASC],   'desc' => ['user.first_name' => SORT_DESC]],
                'last_name'    => ['asc' => ['user.last_name' => SORT_ASC],    'desc' => ['user.last_name' => SORT_DESC]],
                'email'        => ['asc' => ['user.email' => SORT_ASC],        'desc' => ['user.email' => SORT_DESC]],
                'phone_number' => ['asc' => ['user.phone_number' => SORT_ASC], 'desc' => ['user.phone_number' => SORT_DESC]],
                'created_at'   => ['asc' => ['user.created_at' => SORT_ASC],   'desc' => ['user.created_at' => SORT_DESC]],
                'substatus'    => ['asc' => ['user.substatus' => SORT_ASC],    'desc' => ['user.substatus' => SORT_DESC]],
            ]);
    ...
    /**
     * Get tabs data for Call Center (phone-operator only)
     * @return array
     */
    public function getTabsDataForCallCenter(): array
    {
        $currentUserId = Yii::$app->user->id;
        $userRole = Yii::$app->authManager->getRolesByUser($currentUserId);
        $userRoleKey = key($userRole);

        // Only phone-operator and administrators have access
        if (!in_array($userRoleKey, ['phone-operator', 'administrator', 'super-administrator'])) {
            return [];
        }

        $tabs = [];
        $firstGroup = array_key_first(self::CALL_CENTER_STATUS_GROUPS);
        $allUserIds = [];

        foreach (self::CALL_CENTER_STATUS_GROUPS as $groupKey => $groupData) {
            $filterModel = new UserSearch();
            $dataProvider = $this->createDataProviderForCallCenterGroup($groupKey, $filterModel);

            // Count employees in this group
            ...
            if ($userRoleKey == 'phone-operator' ) {
                $countQuery->andWhere(['user.call_center_operator_id' => $currentUserId]);
            }
        ...
        // Load comments for all employees on current page
        $allComments = $this->getCommentsForUsers(array_unique($allUserIds), $currentUserId);
        foreach ($tabs as &$tab) {
            $tab['comments'] = $allComments;
        }
        ...
        // Scheduled calls tab
        $scheduledCount = \common\models\ScheduledCall::find()
            ->where([
        ...
        $hasUpcoming = \common\models\ScheduledCall::find()
            ->where([
                'operator_id' => $currentUserId,
                'is_done'     => 0,
            ])
        ...
        $scheduledLabel = 'Scheduled Calls (' . $scheduledCount . ')';
        $scheduledLabelClass = $hasUpcoming ? 'bg-warning text-dark' : '';

        $tabs[] = [
            'id'           => 'tab-cc-scheduled',
            'group'        => 'scheduled',
            'label'        => $scheduledLabel,
            'labelClass'   => $scheduledLabelClass,
            'dataProvider' => $this->createDataProviderForScheduledCalls($currentUserId),
            'filterModel'  => null,
            'active'       => false,
            'comments'     => [],
            'is_scheduled' => true,
    ...
    private function createDataProviderForScheduledCalls(int $operatorId): ActiveDataProvider
    {
        $query = \common\models\ScheduledCall::find()
            ->alias('sc')
            ->innerJoin('user u', 'u.id = sc.candidate_id')
            ->where([
                'sc.operator_id' => $operatorId,
                'sc.is_done'     => 0,
            ])
            ->with(['candidate'])
            ->orderBy(['sc.scheduled_at' => SORT_ASC]);
        ...
        return new ActiveDataProvider([
            ...
            'sort'       => [
                'attributes'   => [
                    'scheduled_at' => [
                        'asc'  => ['sc.scheduled_at' => SORT_ASC],
                        'desc' => ['sc.scheduled_at' => SORT_DESC],
                    ],
                ],
                'defaultOrder' => ['scheduled_at' => SORT_ASC],
            ],
        ]);
    }
    ...
    /**
     * Create data provider for call center tab group
     * @param string $groupKey
     * @param UserSearch|null $filterModel
     * @return ActiveDataProvider
     */
    private function createDataProviderForCallCenterGroup(string $groupKey, UserSearch $filterModel = null): ActiveDataProvider
    {
        $groupData = self::CALL_CENTER_STATUS_GROUPS[$groupKey];
        ...
        if ($filterModel) {
            ...
            // Phone operator sees only assigned candidates
            if ( $userRoleKey == 'phone-operator') {
                $dataProvider->query->andWhere(['user.call_center_operator_id' => $currentUserId]);
            }
        ...
        // Phone operator sees only assigned candidates
        if ( $userRoleKey == 'phone-operator') {
            $query->andWhere(['user.call_center_operator_id' => $currentUserId]);
        }
    ...
    public function getTabsDataForArchive(): array
    {
        $tabs = [];
        $firstRole = array_key_first(self::ROLE_DISPLAY_NAMES);
        $searchParams = Yii::$app->request->queryParams;

        foreach (self::ROLE_DISPLAY_NAMES as $roleName => $roleDisplayName) {
            $filterModel = new UserSearch();
            $filterModel->load($searchParams);

            $dataProvider = $filterModel->search($searchParams);
    ...
    /**
     * Assign administrator to employee via round-robin (least employees per company)
     * @param int $userId
     * @param int|null $companyId
     * @return bool
     */
    public function assignAdministratorToEmployee(int $userId, ?int $companyId = null): bool
    {
        $administratorId = $this->getNextAdministratorId($companyId);

        if (!$administratorId) {
            Yii::warning("No active administrators found for company_id={$companyId}", 'assignment');
            return false;
        }
        ...
        if ($result) {
            Yii::info("Administrator {$administratorId} assigned to user {$userId} (company_id={$companyId})", 'assignment');
        } else {
            Yii::error("Failed to assign administrator {$administratorId} to user {$userId}", 'assignment');
        }
    ...
    /**
     * Get administrator with least active employees for given company
     * @param int|null $companyId
     * @return int|null
     */
    protected function getNextAdministratorId(?int $companyId): ?int
    {
        $admin = User::find()
            ->alias('u')
            ->innerJoin('auth_assignment aa', 'aa.user_id = u.id')
            ->where([
                'aa.item_name' => 'administrator',
                'u.status'     => User::STATUS_ACTIVE,
            ])
            ->leftJoin(
                'user emp',
                'emp.administrator_id = u.id' .
                ' AND emp.status = ' . User::STATUS_ACTIVE .
                ($companyId ? ' AND emp.company_id = ' . (int)$companyId : '')
            )
            ->select(['u.id', 'COUNT(emp.id) AS employee_count'])
            ->groupBy('u.id')
            ->orderBy(['employee_count' => SORT_ASC, 'u.id' => SORT_ASC])
            ->asArray()
            ->one();
